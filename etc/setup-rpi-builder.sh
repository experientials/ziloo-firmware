...
sudo apt install -y xmlto help2man groff libreoffice-writer vim bash-completion fop dblatex texi2html docbook-utils asciidoc groff  

# Networking support
sudo apt install -y dnsmasq u-boot-tools tftpd-hpa tftp sshpass ssh-askpass socat parprouted dhcp-helper nfs-kernel-server rpcbind

# Disk Image building
sudo apt install -y mtools parted binfmt-support debootstrap dosfstools fdisk gdisk kpartx xxd squashfs-tools u-boot-tools genext2fs xz-utils lz4 liblz4-tool 

# Compiling
sudo apt install -y autoconf libtool automake diffstat libudev-dev libusb-1.0-0-dev gawk \
